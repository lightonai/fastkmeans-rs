...
/// driver's PTX -> SASS JIT step. This sidesteps `CUDA_ERROR_UNSUPPORTED_PTX_VERSION`
/// on hosts whose driver is older than the NVRTC toolkit that built the kernels
/// (for example a CUDA 13.1 toolkit running against a CUDA 13.0 driver). The
/// CUBIN is tied to the exact device architecture passed in `options`
...
fn compile_kernels_cubin(src: &str, options: &[String]) -> Result<Ptx, KMeansError> {
    let csrc = CString::new(src).map_err(|e| {
        KMeansError::InvalidK(format!("CUDA kernel source contains a NUL byte: {e}"))
    })?;
    let prog = nvrtc_result::create_program(&csrc, None)
        .map_err(|e| KMeansError::InvalidK(format!("Failed to create NVRTC program: {e:?}")))?;

    // SAFETY: `prog` is a valid program handle from `create_program`; we destroy
    ...
    if let Err(e) = unsafe { nvrtc_result::compile_program(prog, options) } {
        ...
        return Err(KMeansError::InvalidK(format!(
    ...
    let cubin = unsafe {
        ...
        nvrtc_sys::nvrtcGetCUBINSize(prog, &mut size as *mut usize)
            .result()
            .map_err(|e| KMeansError::InvalidK(format!("nvrtcGetCUBINSize failed: {e:?}")))?;
        let mut buf = vec![0 as c_char; size];
        nvrtc_sys::nvrtcGetCUBIN(prog, buf.as_mut_ptr())
            .result()
            .map_err(|e| KMeansError::InvalidK(format!("nvrtcGetCUBIN failed: {e:?}")))?;
...
impl FastKMeansCuda {
    ...
    pub fn with_config_and_device(
        ...
        // that PTX -- most commonly CUDA_ERROR_UNSUPPORTED_PTX_VERSION when the
        // NVRTC toolkit is newer than the installed driver -- transparently fall
        // back to a device-specific CUBIN, which is loaded directly and skips the
        // PTX JIT. This keeps the GPU path working with no configuration; CPU
        // fallback (handled by the caller) only happens if both routes fail.
